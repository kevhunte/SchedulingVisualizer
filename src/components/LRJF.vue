<template>
<div>
  <h3 class="pb-2">Longest Remaining Job First</h3>

  <h6 class="pb-2">
    This algorithm schedules processes for execution by their burst time in descending order.
  </h6>

  <h6 class="pb-2 animated fadeIn delay-2s">
    The below queue shows the amount of processes waiting to execute.<br>
    Each process executes for 5 seconds.
  </h6>
...
export default {
  ...
  created() {
    if (this.proc) {
      this.localCopy = this.proc.slice().sort((a, b) => a.burstTime < b.burstTime ? 1 : -1)
    }
  },
  mounted() {
